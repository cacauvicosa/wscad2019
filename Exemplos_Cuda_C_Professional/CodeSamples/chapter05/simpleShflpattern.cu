...
__global__ void test_shfl_pattern (int *d_out, int *d_in, int *offset)
{
    int value = d_in[threadIdx.x];
    value = __shfl(value, offset[threadIdx.x], BDIMX);
    d_out[threadIdx.x] = value;
}
...
int main(int argc, char **argv)
{
    ...
    int h_pattern[] = {2,1,0,5,4,3,8,7,6,11,10,9,14,13,12,15};
    ...
    CHECK(cudaMalloc((int**)&d_pattern, nBytes));

    CHECK(cudaMemcpy(d_inData, h_inData, nBytes, cudaMemcpyHostToDevice));
    CHECK(cudaMemcpy(d_pattern, h_pattern, nBytes, cudaMemcpyHostToDevice));
